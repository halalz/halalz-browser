...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, NotAllowedToLoadTest) {
  ...
  console_observer.SetPattern(
      "Not allowed to load local resource: halalz://settings/");
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  ASSERT_TRUE(
      ExecuteScript(active_contents(), "window.open(\"halalz://settings\")"));
  ASSERT_TRUE(console_observer.Wait());
}
...
// This test is for the halalz:// scheme. This is a regression test added with:
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, MAYBE_CrashURLTest) {
  ...
  browser()->OpenURL(
      content::OpenURLParams(GURL("halalz://crash/"), content::Referrer(),
...
// Ex, halalz://settings should be loaded only in normal window because
// chrome://settings is not allowed. When tyring to loading halalz://settings in
// private window, it should be loaded in normal window instead of private
// window.
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       SettingsPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("halalz://settings");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       RewardsPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("halalz://rewards");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       WalletPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("halalz://wallet");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       BraveSyncPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("halalz://sync");
}
